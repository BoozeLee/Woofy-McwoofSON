...
ACTION_REGISTRY = os.path.join(os.path.dirname(__file__), '..', 'src', 'action_registry.py')
...
def get_registry_actions():
    ...
    if hasattr(module, 'ACTION_REGISTRY'):
        ...
        raise AssertionError("No ACTIONS or ACTION_REGISTRY found in src/action_registry.py")
...
def test_adr_and_registry_actions_are_in_sync():
    ...
    if missing_in_adr:
        msg += f"Actions in action registry but missing from ADR index: {missing_in_adr}\n"
