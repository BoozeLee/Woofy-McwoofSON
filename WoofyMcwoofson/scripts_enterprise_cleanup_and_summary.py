...
with open(SUMMARY_FILE, "w") as f:
    ...
    write_section(
        f, "Onboarding & Knowledge Vault", [r"onboard.*\.md", r"knowledge-vault_.*\.md"]
    )
    write_section(
        f,
        "Security, Token, & Compliance",
        [r"security.*\.md", r"token.*\.md", r"remediation.*\.md", r"audit.*\.md"],
    )
    write_section(
        f, "CI/CD & GitHub Workflows", [r"\.github/workflows/.*\.yml", r"ci.*\.yml"]
    )
    write_section(
        f,
    ...
    write_section(
        f,
        "Status, Handoff, Transition, Launch",
        [r"status.*\.md", r"handoff.*\.md", r"transition.*\.md", r"launch.*\.md"],
    )
